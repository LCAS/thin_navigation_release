...
namespace thin_navigation{
  ...
  void ROSLocalizer::initGUI(){
    _use_gui=true;
    cv::namedWindow( "thin_localizer", 0 );
    cv::setMouseCallback( "thin_localizer", &ROSLocalizer::onMouse, this );
    cerr << "GUI initialized" << endl;
  }

  void ROSLocalizer::rangesToEndpoints(Vector2fVector& endpoints,
 ...
 //      cv::putText(img, status_str, cv::Point(10, 10), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(200,0,200), 1);
  ...
  void ROSLocalizer::subscribeCallbacks(const std::string& laser_topic){
    ...
    _ranges_pub = _nh.advertise<thin_navigation::LocalizerRanges>("localizer_ranges", 2);
